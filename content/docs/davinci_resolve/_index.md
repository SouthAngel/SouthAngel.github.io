---
---
*吐槽：达芬奇的api真是五脏不全，说是同时支持lua python javascript，只有lua是软件内置的，相当于是另起了一个客户端程序与fuscript通信交互，为此甚至在软件安装目录中带了一整个electorn，如果你是基于py或js开发，基本上可以说是与达芬奇软件本身没有什么关系了，虽说做到了软件与插件解耦，但会使你的插件不得不使用许多外部依赖，结果就是你想写一个小插件最后也会变的臃肿不堪。这些暂且不论，主要问题是核心api函数许多细节交互都没有，你没办法通过api做一些细致的控制，这个软件对开发者来说真是体验很差。*
## API功能缺失
1. 无法获取用户选择
1. 无法添加右键菜单项
1. 无法获取 In Out 标记
